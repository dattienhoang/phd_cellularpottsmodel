...
/*** Global Parameters ***/
  ...
  int seed = 				3; 			// use 1 for same sequence every time; use 0 for time-based randomness
  ...
  /*** Energies ***/
  const double beta =		1.0;		//HUGE_VAL, 10, 1
  const double J_air =		1.0;
  const double J_cel =		100.0;
  const double J_col =		0.0;
  const double L_vol = 		1.0;  	//  penalty for deviating volume
  const double L_ani =		0.0; 		//  penalty for high anisotropy
  const double L_blb =		5000.0;  		//  penalty for wiggliness
