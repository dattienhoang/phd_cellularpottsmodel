...
int flip()
{
  ...
  //  double deltaBlobularEnergy = 0.0;
    double deltaRatioEnergy = 0.0;
    double deltaInteractionEnergy = 0.0;
    double deltaVolumeEnergy = 0.0;

    
    //eventually find a correct way to make a less computationally expensive method of calculations for calculating change in interaction energy
    
  if(oldCell!=0){
    deltaVolumeEnergy -= volumeEnergy(oldCell);
 //   deltaEnergy -= anisotropyEnergy(oldCell);
 //   deltaBlobularEnergy -= blobularEnergy(oldCell);
    deltaInteractionEnergy -= interactionEnergy(oldCell);
    deltaRatioEnergy -= ratioEnergy(oldCell);
  }
 ...
 //   deltaBlobularEnergy -= blobularEnergy(newCell);
    deltaInteractionEnergy -= interactionEnergy(newCell);
    deltaRatioEnergy -= ratioEnergy(newCell);
  ...
  if( deltaEnergy < 0 ){
   ...
   //   totalBlobularEnergy+=deltaBlobularEnergy;
      totalRatioEnergy+=deltaRatioEnergy;
...
void choose()
{
  ...
  do{
    ...
    // Keep trying until you don't get a neighbor that's of a different type than yourself
    // TODO: this is inefficient...should remove a direction already tried from the set of remaining directions to be tried
    // newCell invades oldCell
